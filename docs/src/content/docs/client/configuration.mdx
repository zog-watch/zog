---
title: Configuration
description: Configuration reference for the Zog client.
---

The config for the Zog can be provided in 2 different ways, depending on how you are hosting Zog:
...
- If you are hosting Zog using shared hosting (such as cPanel or FTP), please use [the config file](#method-2-config-file).

Both methods can specify any of the keys listed in the [Shared Config](#config-reference-shared-config) section.

## Method 1 - Environment Variables

The Zog client can be configured using environment variables **at build time**. You cannot use this method if hosting the pre-built `Zog.zip` files!

Using environment variables to configure Zog also allows configuration of some [environment variable specific keys](#config-reference-environment-variables-only).

## Method 2 - Config File

When using the pre-built `Zog.zip`, you can set the configuration in the `config.js` file.
...
This is the **read** API key from TMDB to allow Zog to search for media. [Get one by following our guide](/client/tmdb).
...
This is the URL for the Zog backend server which handles cross-device syncing.

The backend server can be found at https://github.com/zog-watch/backend and is offered as a [Docker](https://docs.docker.com/get-started/overview/) image for deployment.
...
- Type: `string`
- Default: `"https://chromewebstore.google.com/detail/Zog-extension/hoffoikpiofojilgpofjhnkkamfnnhmm"`
...
- Type: `string`
- Default: `"https://addons.mozilla.org/en-GB/firefox/addon/Zog-extension"`
- Example: `"https://google.com"`
